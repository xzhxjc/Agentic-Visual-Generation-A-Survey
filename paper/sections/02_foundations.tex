...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/ch2_l1l5.png}
  \caption{L1--L5 autonomy levels. Each level adds a span of state-dependent control on top of the previous one: fixed pipelines (L1), tool and role selection (L2), observation-conditioned revision (L3), long-horizon recovery and adaptive stopping (L4), and cross-task improvement under held-out evaluation (L5). The dashed trajectory emphasizes that the levels are cumulative; a system qualifies as AVG when its evidence supports L3 or higher.}
  \label{fig:ch2-l1l5}
\end{figure}
